...
import { ArrowRight, ShoppingBasket, Tractor, Wheat, Apple, Carrot, Sparkles, Truck, IndianRupee } from "lucide-react";
    ...
    <Layout>
      ...
      <section id="about" className="container py-20">
        ...
        <div className="grid md:grid-cols-3 gap-6 max-w-5xl mx-auto">
          {[
            { icon: Sparkles, title: "Fresher Food", color: "text-primary" },
            { icon: IndianRupee, title: "Better Prices", color: "text-secondary" },
            { icon: Truck, title: "Direct Connection", color: "text-amber-600 dark:text-amber-400" },
          ].map((w, i) => (
            <motion.div key={w.title} {...fade} transition={{ delay: i * 0.1 }} className="rounded-2xl bg-card border border-border p-6 hover-lift">
              ...
              <p className="text-sm text-muted-foreground mt-2">Built on transparency, trust, and traceability — from sapling to plate.</p>
            </motion.div>
          ))}
        </div>
      </section>
